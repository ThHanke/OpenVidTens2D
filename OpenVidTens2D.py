...
class InitFrame(wx.Frame):
    def __init__(self, parent):
        ...
        wx.Frame.__init__(self, None, wx.ID_ANY, title='Test', pos=(
            int(screensize[2] / 2 - self.winsize[0] / 2), int(screensize[3] / 2 - self.winsize[1] / 2)),
            size=self.winsize,
            style=wx.BORDER_RAISED | wx.STAY_ON_TOP)
        self.logopanel = wx.Panel(self, wx.ID_ANY, style=wx.BORDER_RAISED)
        wx.StaticBitmap(self.logopanel, -1, wx.Image('logoOpenVidTens.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap(),
                        (0, 0))
        ...
        self.status.progressbar = wx.Gauge(self.status, -1, 100, wx.Point(rect.x + 2, rect.y + 2),
                                           wx.Size(int(self.winsize[0] / 3) - 4, rect.height - 4))
        ...
        parent.LiveTrackWin = LiveTrackWin.LiveTrackWin(self.totrackqueue, self.trackresultqueue, self.tracktocam,
                                                        self.tracktoplot)
